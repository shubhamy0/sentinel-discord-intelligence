"""
Database layer for the Sentinel Activity Tracker.
...
logger = logging.getLogger('sentinel.tracker.db')
...
def _add_column_if_missing(conn: sqlite3.Connection, table: str, column: str, col_type: str) -> None:
    """
    Safely add a column to an existing table only if it does not already exist.
    """
    cursor = conn.cursor()
...
def init_db_sync(db_path: str) -> None:
    ...
    try:
        with conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_message_guild_user ON message_activity(guild_id, user_id);")

        _add_column_if_missing(conn, "message_activity", "content", "TEXT")
        _add_column_if_missing(conn, "message_activity", "message_url", "TEXT")
...
def log_message_sync(
    ...
    try:
        with conn:
            ...
            conn.execute(
                """
                INSERT OR IGNORE INTO message_activity
                    (message_id, user_id, timestamp, guild_id, channel_id, content, message_url)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (message_id, user_id, timestamp, guild_id, channel_id, content, message_url)
            )
...
async def log_message(
    ...
    await asyncio.to_thread(
        log_message_sync, db_path, message_id, user_id, username,
        timestamp, guild_id, channel_id, content, message_url
    )
...
def find_user_by_name_sync(db_path: str, username_query: str) -> Optional[Dict[str, Any]]:
    """
    Synchronously find a user by their username.
    """
    conn = _connect(db_path)
    try:
        cursor = conn.cursor()
...
def add_tracked_user_sync(db_path: str, user_id: int) -> None:
    """
    Synchronously add a user ID to tracked_users.
    """
    conn = _connect(db_path)
    try:
        with conn:
...
async def is_tracked_user(db_path: str, user_id: int) -> bool:
    """
    Asynchronously check if a user is tracked.
    """
    return await asyncio.to_thread(is_tracked_user_sync, db_path, user_id)

def fetch_messages_for_user_sync(
    db_path: str,
    user_id: int,
    guild_id: int,
    limit: int = 1000,
    ...
    """
    Synchronously fetch stored messages for a specific user within a guild.
    """
    conn = _connect(db_path)
    try:
        cursor = conn.cursor()
        cursor.execute(
            """
            SELECT message_id, channel_id, timestamp, content, message_url
        ...
        logger.exception("Failed to fetch messages for user sync: %s", e)
        return []
    finally:
        conn.close()
...
async def fetch_messages_for_user(
    ...
    """
    Asynchronously fetch stored messages for a specific user within a guild.
    """
    return await asyncio.to_thread(fetch_messages_for_user_sync, db_path, user_id, guild_id, limit)
