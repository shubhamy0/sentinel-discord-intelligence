"""
Cog implementing the Sentinel Activity Tracker listeners and commands.
Tracks message sending, voice connections, and generates telemetry reports.
"""
...
logger = logging.getLogger('sentinel.tracker.cog')
...
def draw_ascii_heatmap(data: List[Tuple[int, int, int]]) -> str:
    """
    Generate an ASCII-art representation of the activity heatmap.
...
class ActivityTracker(commands.Cog):
    ...
    async def subscribe_to_all_tracked_users(self) -> None:
        """Fetch all persistently tracked user IDs and verify connections."""
        await self.bot.wait_until_ready()
        try:
            user_ids = await get_tracked_users(self.db_path)
            if not user_ids:
                return
            logger.info("Tracking active presence for %d registered users.", len(user_ids))
        except Exception as e:
    ...
    async def on_ready(self) -> None:
        """Re-verify tracked users on startup or reconnection."""
        self.bot.loop.create_task(self.subscribe_to_all_tracked_users())

    @commands.Cog.listener()
    async def on_message(self, message: discord.Message) -> None:
        """Record messages sent by users, including content and URL for Sentinel analysis."""
        ...
        self.bot.loop.create_task(
            log_message(
                ...
                message_url=message.jump_url or None,
            )
        )
    ...
    async def trackpresence(self, ctx: commands.Context, *, user_query: str) -> None:
        ...
        await add_tracked_user(self.db_path, user_id)
        await ctx.send(f"✅ Added **{username}** (ID: `{user_id}`) to presence tracking list.")
